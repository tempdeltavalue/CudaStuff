...
//	const int triCount = 3; // vert count
//	const int cubVertCount =cubeVec.size();
//
//	vec3* g_vertex_buffer_data = new glm::vec3[cubVertCount * triCount * cubesCount];
//	vec3* g_color_buffer_data = new glm::vec3[cubVertCount * triCount * cubesCount];
//
//
//	int intend = triCount * cubVertCount;
//
//	std::vector<vec3> colours = { vec3(1, 0, 0), vec3(0, 1, 0), vec3(0, 0, 1) };
//
//	for (int y = 0; y < cubVertCount; y++) {
//		int index = y;// * 3;
//		g_vertex_buffer_data[index] = glm::vec3(cubeVec[y][0], cubeVec[y][1], cubeVec[y][2]);
//
//		if (index > 6) {
//			g_color_buffer_data[index] = vec3(1, 0, 0);
//
//		} else {
...
//
//		// Draw the triangle !
//		glDrawArrays(GL_TRIANGLE_STRIP_ADJACENCY, 0, cubVertCount* cubesCount); // 12*3 indices starting at 0 -> 12 triangles
//
